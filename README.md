...
| **WSL 2** _(Windows only)_             | `wsl --install` as administrator → reboot → enable the WSL backend in Docker Desktop settings. |
...
| Telegram                         | `TELEGRAM_ENABLED` | `TELEGRAM_BOT_TOKEN` _(adapter arrives in Phase 2)_                                                                                                                     |
...
| Show URLs                           | `docker compose exec web python manage.py show_urls` _(if installed)_ or read `config/urls.py` |
